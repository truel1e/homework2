...
def Dijkstra_fast(graph, start): #Алгоритм Дейсктры с использованием двоичной кучи
    ...
    distances[start] = 0 #Инициализируем значение начального узла 0
    heap.push((0, start))
...
#Графики зависимости времени выполнения от количества вершин для обоих алгоритмов
#Построим пару промежуточных функций
def dijkstra_slow_time(graph, start): #Вычисляем время выполнения Dijkstra_slow
    start_time = time.time()
    previous_nodes, shortest_path = Dijkstra_slow(graph, start)
    end_time = time.time()
    return end_time - start_time

def dijkstra_fast_time(graph, start): #Вычисляем время выполнения Dijkstra_fast
    start_time = time.time()
    distances = Dijkstra_fast(graph, start)
    end_time = time.time()
    return end_time - start_time

A = 1000

# Список размеров графов
sizes = [i * A for i in range(1,3)]

# Списки времен выполнения алгоритмов для каждого размера графа
dijkstra_slow_times = []
dijkstra_fast_times = []

# Измеряем время выполнения алгоритмов для каждого размера графа
for size in sizes:
    graph = generate_graph(size, 0.4)
    start_node = '0'
    dijkstra_slow_times.append(dijkstra_slow_time(graph, start_node))
    dijkstra_fast_times.append(dijkstra_fast_time(graph, start_node))

# Строим графики зависимости времени выполнения алгоритмов от количества вершин
plt.plot(sizes, dijkstra_slow_times, label='Dijkstra_slow')
plt.plot(sizes, dijkstra_fast_times, label='Dijkstra_fast')
plt.legend()
